...
def on_connection(client, user_data, flag, rc):
    ...
    conn_text = ("System>>{}\n Your connection to broker has been successfully established\n\n".format(client_id, status_decoder.get(rc)))
...
window = Tk()
window.title("IoT Messenger Client2")
window.minsize(600, 400)
window.resizable(0, 0)

Frame1 = LabelFrame(window, text=" Window", width=600, height=300,bg='Grey')
Frame1.place(y=0, x=0)
Frame2 = LabelFrame(window, text="Enter Message", width=600, height=100,bg='Ivory3')
...
ChatFill.configure(state="disabled",bg='AntiqueWhite2')
...
SendButton = Button(Frame2, text="Send", command=send_message, bg='Grey')
